...
#%%
#def make_bar_chart2(feature):
    #balti_chart = [balti[balti['District']!=""][feature].value_counts()]
    #balti_chart = pd.DataFrame(balti_chart)
    #balti_chart.index = ["District"]
    #balti_chart.plot(kind='bar', stacked=False, figsize=(10,5))
#print(balti.head())
#make_bar_chart2("")
...
#%%
px.set_mapbox_access_token("pk.eyJ1IjoieGhhbGxvcmFuIiwiYSI6ImNrOTNpMXZsYzAyaTgzZ21zMDB2cHdrNHkifQ.NdO0vx_uFnuf2KtPhDfR1w")
# df = px.data.carshare()
fig1 = px.scatter_mapbox(balti_loca[(balti_loca["Sex"]=="M")], lat="Latitude", lon="Longitude", color="Charge", color_continuous_scale=px.colors.cyclical.IceFire, size_max=1, zoom=10.2)
fig2 = px.scatter_mapbox(balti_loca[(balti_loca["Sex"]=="F")], lat="Latitude", lon="Longitude", color="Charge", color_continuous_scale=px.colors.cyclical.IceFire, size_max=1, zoom=10.2)
...
#fig4 = px.scatter_mapbox(balti_loca[(balti_loca["Charge"]=="11415") & (balti_loca["Sex"]=="F")], lat="Latitude", lon="Longitude", color="year", color_continuous_scale=px.colors.cyclical.IceFire, size_max=1, zoom=11)
#fig5 = px.scatter_mapbox(balti_loca[(balti_loca["Charge"]=="11415") & (balti_loca["Sex"]=="F")], lat="Latitude", lon="Longitude", color="year", color_continuous_scale=px.colors.cyclical.IceFire, size_max=1, zoom=11)
...
# %%
#KNN
top_5_balti=top_5_balti.dropna()
ychargedf=top_5_balti['Charge']
#ychargedf=pd.DataFrame(data=ychargedf)
print(ychargedf)
#len(ychargedf)
#%%

xcharge=top_5_balti[["Age","Max.TemperatureF","Mean.TemperatureF", "Min.TemperatureF","PrecipitationIn","Sex_code","Daynight_Code"]]
ycharge=pd.to_numeric(ychargedf)
#xcharge=pd.to_numeric(xcharge)
#print(ycharge)
...
cv_results = cross_val_score(knn_cv, xcharge, ycharge, cv=5)
print(cv_results) 
np.mean(cv_results) 
# %%
from sklearn.neighbors import KNeighborsClassifier
knn = KNeighborsClassifier(n_neighbors=mrroger) # instantiate with n value given
knn.fit(xcharge,ycharge)
y_pred = knn.predict(xcharge)
print(y_pred)
knn.score(xcharge,ycharge)
